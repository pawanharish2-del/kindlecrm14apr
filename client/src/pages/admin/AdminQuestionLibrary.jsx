...
const AdminQuestionLibrary = () => {
    ...
    const fetchLibrary = async () => {
        try {
            ...
            if (!data || Object.keys(data).length === 0) {
                ...
                data = { Female: {}, Male: {}, General: {} };
            }
            // Ensure default roots exist, avoiding undefined loops
            if (!data.Female) data.Female = {};
            ...
            const femaleCats = Object.keys(data.Female);
            if (femaleCats.length > 0) {
                setActiveCategory(femaleCats[0]);
            }
        } catch (err) {
    ...
    const handleAddCategory = () => {
        ...
        if (!cat) return;
        if (libraryData[genderTab] && libraryData[genderTab][cat]) {
            alert("Category already exists for " + genderTab);
            return;
        }

        const newLib = { ...libraryData };
        if (!newLib[genderTab]) newLib[genderTab] = {};
        newLib[genderTab][cat] = [];
    ...
    const handleAddQuestion = () => {
        ...
        const newLib = { ...libraryData };
        if (!newLib[genderTab][activeCategory]) {
            newLib[genderTab][activeCategory] = [];
        }
    ...
    const handleDeleteQuestion = (cat, index) => {
        if (window.confirm("Are you sure you want to delete this question?")) {
            const newLib = { ...libraryData };
            newLib[genderTab][cat].splice(index, 1);
            setLibraryData(newLib);
        }
    };
    ...
    const currentCategories = libraryData[genderTab] || {};
    ...
    return (
        <div className="ql-admin-body">
            ...
            {/* Gender Navbar */}
            <div className="gender-navbar">
                {['Female', 'Male', 'General'].map(g => (
                    <div
                        key={g}
                        className={`gender-tab ${genderTab === g ? 'active' : ''}`}
                        onClick={() => {
                            setGenderTab(g);
                            const cats = Object.keys(libraryData[g] || {});
