...
// Admin Signup Route — Only creates Super Admin accounts
router.post('/signup', async (req, res) => {
    try {
        ...
        const admin = new User({
            name, email, password, phone: phone || '', role: 'superadmin'
        });
        ...
        res.status(500).json({ success: false, message: 'Error creating superadmin', error: error.message });
    }
});
    ...
    try {
        ...
        res.json({
            ...
            user: {
                ...
                dashboardPath: userRoleObj ? userRoleObj.dashboardPath : '/superadmin',
...
router.put('/users/:userId', verifyAdminOrSuperAdmin, async (req, res) => {
    try {
        ...
        if (roleId && String(roleId) !== String(user.role)) {
            ...
            roleChanged = true;
        } else if (user.role && user.role !== 'superadmin') {
