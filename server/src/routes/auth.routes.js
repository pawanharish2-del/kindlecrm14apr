...
async function buildUserResponse(user) {
  ...
  return {
    ...
    roleId: user.role, // ObjectId or 'superadmin'
...
router.post('/login', async (req, res) => {
  try {
    ...
    // SuperAdmins must use the superadmin login page
    if (user.role === 'superadmin') {
      return res.status(403).json({ success: false, message: 'SuperAdmins must use the superadmin login page' });
    }
    ...
    if (roleData.name && (roleData.name.toLowerCase() === 'admin' || roleData.name.toLowerCase() === 'superadmin')) {
      return res.status(403).json({ success: false, message: 'SuperAdmins must use the superadmin login page' });
    }
